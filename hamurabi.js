...
function writeGameState() {
  document.getElementById("year").innerHTML =
    "Rapport för år: " + gameState.year;
  document.getElementById("starved").innerHTML =
    "Ihjälsvultna: " + gameState.starved;
  document.getElementById("newcomers").innerHTML =
    "Inflyttade: " + gameState.newcomers;
  document.getElementById("population").innerHTML =
    "Befolkning: " + gameState.population;
  document.getElementById("acres").innerHTML = "Tunnland: " + gameState.acres;
  document.getElementById("bushels").innerHTML =
    "Skäppor: " + gameState.bushels;
...
function gameStart() {
  ...
  plantSlider.min = 0;
  plantSlider.max = minimum(
    gameState.internalAcres,
    10 * gameState.population,
    gameState.internalBushels
  );
  plantSlider.value = 0;
  plantSlider.step = 1;
...
function updateInternalAcres() {
  gameState.internalAcres =
    gameState.acres + parseInt(getSlider("sliderAcrestosellbuy").value);
}
...
function updateInternalBushels() {
  ...
  gameState.internalBushels =
    gameState.bushels -
    acresToSellOrBuy * gameState.price -
    bushelsForFeeding -
    bushelsForPlanting;
}
...
function updateMaximumAcresToSellBuy() {
  var acresSlider = getSlider("sliderAcrestosellbuy");
  acresSlider.max =
    Math.floor(gameState.internalBushels / gameState.price) +
    parseInt(acresSlider.value);
}
...
function updateMaximumPlantWithSeed() {
  ...
  plantSlider.max = minimum(
    gameState.internalAcres,
    10 * gameState.population,
    gameState.internalBushels + parseInt(plantSlider.value)
  );
}
...
function onChangeAcres(elementId) {
  if (elementId === "minusbuttonAcrestosellbuy") {
    getSlider("sliderAcrestosellbuy").value =
      parseInt(getSlider("sliderAcrestosellbuy").value) - 1;
...
function onChangeFeeding(elementId) {
  if (elementId === "minusbuttonFeedpeople") {
    getSlider("sliderFeedpeople").value =
      parseInt(getSlider("sliderFeedpeople").value) - 1;
...
function onChangePlanting(elementId) {
  if (elementId === "minusbuttonPlantwithseed") {
    getSlider("sliderPlantwithseed").value =
      parseInt(getSlider("sliderPlantwithseed").value) - 1;
...
function calculateStarvedPeople() {
  var numberPeopleStarved =
    gameState.population -
    Math.floor(parseInt(getSlider("sliderFeedpeople").value) / 20);
...
function calculateNewcomers() {
  return (
    Math.floor(
      (20 * gameState.internalAcres + gameState.internalBushels) /
        (100 * gameState.population)
    ) + 1
  );
}
...
function tooManyPeopleStarved() {
  var originalPopulation =
    gameState.population + gameState.starved - gameState.newcomers;
...
function finishTurn() {
  ...
  gameState.bushels =
    gameState.internalBushels +
    gameState.harvest * parseInt(getSlider("sliderPlantwithseed").value);
  ...
  if (gameState.year < 10 && !tooManyPeopleStarved()) {
    ...
    alert(
      "Du har orsakat att 45% av befolkningen svultit ihjäl!\n" +
        "Du har avsatts.\n" +
        "Försök igen."
    );
    gameStart();
    ...
    alert(
      "Total prestation\n" +
        "Genomsnittligt antal ihjälsvultna per år: " +
        averagePeopleStarved +
        "\n" +
        "Befolkning: " +
        gameState.population +
        "\n" +
